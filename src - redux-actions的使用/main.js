    ...
    */
     ...
     */
      ...
      */    
       ...
       *            频繁引入store
       *            每个组件都需要监听
       *                ----- react-redux (可以解决以上三个问题)
       *                       npm i react-redux -S
       *                        用来辅助redux，不能单独使用，需要配合redux一起使用
       *                      由两部分组成：
       *                        1.provider 原理：context
       *                        2.connect  原理：高阶组件
       *            无法管理多个store
       *                -----combineReducers:管理多个reducer 等价于vuex的modules
       ...
       */
        ...
        *           一般情况下会在遍历的时候出现
        *       注意：
        *           key值必须是唯一的，且不可更改（最好不要更改)
        *   首先，key值不是给开发者使用的，而是给虚拟dom使用。因为虚拟dom中会有一个diff算法，diff算法在进行新旧两个虚拟dom
        *   对比的时候，如果当前数据是通过遍历得到的，则会比较每一个节点的key值，如果key值不一样，则会进行组件的卸载或者标签
        *   移除，如果一样，则只需要更新组件或标签中的key值
        * 
        *   key值最好不要用数组的下标，要用一个唯一的标识，最好后端提供，比如id
        *   
        *   如果组件中的key值发生了改变，会执行哪些生命周期函数？
        ...
        */
         ...
         *  PureCompPonent
         *      
         *      1.通常创建组件的时候是通过es6的类的方法继承于React.Component来进行组件的创建，现在我们
         *      可以通过继承REact.PureComponent的方式来创建组件
         *      2.PureCompPonent创建出来的组件当数据发生改变时默认会对数据进行一个浅比较，如果数据更改与之前的
         *      相同则不更新render函数，反之则更新
         *      
         *      注意：只要用了PureCompPonent就不要再写shouldComponentUpdate这个生命周期，会报错
         *      使用的场景：一般情况下PureCompPonent都用来做展示组件/ui组件
         * 
         */

         /**
          *  redux-actions
          *       安装： npm i redux-actions -S
          *       可以用来精简redux中的代码
          * 
          */
         /**
         ...
         /**
          *     插槽
          *         在react中组件当作标签使用的时候，如果在组建标签内部嵌套一些元素或者标签的时候，如何进行显示？
